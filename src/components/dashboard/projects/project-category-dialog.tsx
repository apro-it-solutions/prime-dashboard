...
type ProjectCategoryFormValues = z.infer<typeof projectCategorySchema>
...
type ProjectCategoryDialogProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  /** Category being edited; omit or pass null to create a new one. */
  editing?: ProjectCategory | null
  /** Called with the newly created category — used to auto-select it in the project form. */
  onCreated?: (category: ProjectCategory) => void
}

/**
 * Create/edit dialog for a Projects-only category. Shared by the project
 * category management page and the inline "New" button on the project form, so
 * an admin never has to leave the Projects section to add one.
 */
export function ProjectCategoryDialog({
  ...
  const onSubmit = (values: ProjectCategoryFormValues) => {
    if (editing) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{editing ? 'Edit' : 'Add'} Project Category</DialogTitle>
          <DialogDescription>
            Project categories are separate from the blog and product categories
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder='e.g. Network Monitoring' {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
